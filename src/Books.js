...
class BestBooks extends React.Component {
  ...
  handelUpdateModal = (e) => {
    ...
    console.log(
      `${process.env.REACT_APP_API_URL}/booksCollection/${this.state.selectedBookDataObj._id}`
    );
    axios
      .put(
        `${process.env.REACT_APP_API_URL}/booksCollection/${this.state.selectedBookDataObj._id}`,
  ...
  componentDidMount = () => {
    axios
      .get(
        `${process.env.REACT_APP_API_URL}/booksCollection?email=${this.props.auth0.user.email}`
      )
      .then((bookResponse) => {
